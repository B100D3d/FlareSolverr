...
type OverridesProps = "method" | "postData" | "headers"

// We always set a Windows User-Agent because ARM builds are detected by Cloudflare
const DEFAULT_USER_AGENT =
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.138 Safari/537.36"

async function resolveChallengeWithTimeout(
    ctx: RequestContext,
    params: BaseRequestAPICall,
    page: Page
) {
    ...
    try {
        ...
        return await Promise.race([
            ...
            timer.set(
                maxTimeout,
                `Maximum timeout reached. maxTimeout=${maxTimeout} (ms)`
            ),
        ])
    } finally {
        timer.clear()
...
async function resolveChallenge(
    ctx: RequestContext,
    { url, proxy, download, returnOnlyCookies }: BaseRequestAPICall,
    page: Page
): Promise<ChallengeResolutionT | void> {
    let status = "ok"
    let message = ""
    ...
    if (returnOnlyCookies) {
        ...
        if (download) {
            ...
            payload.result.response = (await response.buffer()).toString(
                "base64"
            )
        } else {
...
function mergeSessionWithParams(
    { defaults }: SessionsCacheItem,
    params: BaseRequestAPICall
): BaseRequestAPICall {
    ...
    copy.headers =
        { ...(defaults.headers || {}), ...(params.headers || {}) } || null
...
async function setupPage(
    ...
    if (headers) {
        ...
        overrideResolvers.headers = (request) =>
...
const browserRequest = async (
    ...
    if (session === false) {
        return ctx.errorResponse(
            "This session does not exist. Use 'list_sessions' to see all the existing sessions."
        )
    }
    ...
    let page
    try {
        page = await setupPage(ctx, params, session.browser)
        ...
        log.error(error)
        return ctx.errorResponse(
            "Unable to process browser request. Error: " + error
        )
    } finally {
        if (page && !page.isClosed()) {
            log.info(`Closing the page in browserRequest finally block`)
            await page.close()
        }
        if (oneTimeSession) {
...
export const routes: Routes = {
    "sessions.create": async (
        ctx,
        { session, ...options }: SessionsCreateAPICall
    ) => {
    ...
    "request.get": async (ctx, params: BaseRequestAPICall) => {
        ...
        if (params.postData) {
            return ctx.errorResponse(
                'Cannot use "postBody" when sending a GET request.'
            )
        }
    ...
    "request.post": async (ctx, params: BaseRequestAPICall) => {
        ...
        if (!params.postData) {
            return ctx.errorResponse(
                'Must send param "postBody" when sending a POST request.'
            )
        }
