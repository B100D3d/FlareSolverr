...
export default async function resolveChallenge(
    url: string,
    page: Page,
    response: Response
): Promise<Response> {
    ...
    if (response.status() > 400) {
        ...
        for (const selector of CHALLENGE_SELECTORS) {
            ...
            if (cfChallengeElem) {
                selectorFoundCount++
                log.debug(
                    `Javascript challenge element '${selector}' detected.`
                )
        ...
        log.debug(
            "Javascript challenge selectors found: " +
                selectorFoundCount +
                ", total selectors: " +
                CHALLENGE_SELECTORS.length
        )
    } else {
        // some sites use cloudflare but there is no challenge
        log.debug(
            `Javascript challenge not detected. Status code: ${response.status()}`
        )
    ...
    if (await page.$('input[name="cf_captcha_kind"]')) {
        ...
        if (captchaSolver) {
            ...
            if (challengeForm) {
                const captchaTypeElm = await page.$(
                    'input[name="cf_captcha_kind"]'
                )
                const cfCaptchaType: string = await captchaTypeElm.evaluate(
                    (e: any) => e.value
                )
                const captchaType: CaptchaType = (CaptchaType as any)[
                    cfCaptchaType
                ]
                if (!captchaType) {
                ...
                if (
                    ...
                    sitekey = await sitekeyElem.evaluate((e) =>
                ...
                for (const name of TOKEN_INPUT_NAMES) {
                    ...
                    if (input) {
                        ...
                        log.debug(
                            `Challenge response field '${name}' found in challenge form.`
                        )
                        await input.evaluate(
                            (e: HTMLTextAreaElement, token) => {
                                e.value = token
                            },
                            token
                        )
                ...
                if (responseFieldsFoundCount == 0) {
                    throw new Error(
                        "Challenge response field not found in challenge form."
                    )
                }
                ...
                if (await page.$('input[name="cf_captcha_kind"]')) {
                    throw new Error(
                        "Captcha service failed to solve the challenge."
                    )
                }
            }
        } else {
